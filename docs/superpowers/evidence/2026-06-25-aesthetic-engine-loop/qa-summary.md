...
- 页面唯一 `h1` 为“敬请期待”。
- 初始静止态不渲染 `main img`、`source-card` 或 `trail-item`，页面中间只保留提示语。
- 鼠标移动后才生成图片拖尾项；桌面拖尾布局为约 `166×250`，移动端约 `121×181`。
- 快速鼠标移动后所有拖尾项仍使用同一固定 2:3 布局容器，`object-fit: cover`，CSS 缩放固定为 `1`；旋转只改变视觉外接框。
