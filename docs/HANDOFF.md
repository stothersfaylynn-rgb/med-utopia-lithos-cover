...
- 初始静止态只显示中间提示语，不渲染任何图片卡片或源图片。
- 参考 CodePen 的鼠标图片拖尾机制，但使用项目内医学主题资产，不复制参考页图片或品牌内容；只有鼠标移动后才生成拖尾图片。
- 拖尾图片采用固定 2:3 竖向卡片系统，桌面约 `166×250` 布局尺寸，移动约 `121×181`；CSS 缩放固定为 `1`，旋转只影响视觉外接框，不改变布局尺寸。
...
- 初始静止态不渲染可见图片；快速鼠标移动后拖尾项仍使用固定 2:3 布局容器且尺寸统一；reduced-motion 下不生成连续拖尾项。
